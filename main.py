...
def verify_msg(guildname, domains):
    if domains == "":
        ...
        return "Thank You for giving your ** @{} ** ID to verify yourself on {}.".format(domains, guildname)
...
async def on_ready():
    ...
    await client.change_presence(activity=discord.Game(name='CODEs to keep this server safe'))
...
async def on_message(message):
    ...
    msg = message.content
    id= ['@niamt.ac.in']
    if (any(word in msg for word in id)):
        check_on_join = get_guild(ctx.guild.id)
        if check_on_join == None:
            new_guild(ctx.guild.id)
        check_on_join = get_guild(ctx.guild.id)
        ...
        if user_prev_verify == None:
            ...
            await ctx.channel.send(verify_msg(ctx.guild, check_on_join[1]))
        elif user_prev_verify[4] == 0:
            await ctx.channel.send(verify_msg(ctx.guild, check_on_join[1]))
    
    message_content = message.content.strip()
    if (message.guild) and email_check(message_content):
        ...
        if len(users_guilds) >= 1:
            ...
            if len(verif_list) >= 1:
                ...
                emailmessage = Mail(
                    ...
                    subject=f'Verification Code for {message.guild} discord server',
                    html_content=f'Your verification code is for {message.guild} discord server is {str(random_code)}.\nDo not reply back to this e-mail for queries.\n Contact Server Admins or Moderators for Help. \nHave a Good Day !!')
                try:
                    sg = SendGridAPIClient(os.environ.get('SENDGRID_API_KEY'))
                    response = sg.send(emailmessage)
                    await message.channel.send("An Email has been sent to this Email Id. **Reply here with your 6 digit verification code**. If you haven't received it, check your spam folder.")
        ...
        if len(prev_codes) >= 1:
            for i in prev_codes:
                ...
                if role:
                    ...
                    if role not in member.roles:
                        await member.add_roles(role)
                        role_get = discord.utils.get(ctx.guild.roles, name="Unverified")  
                    ...
                    await member.add_roles(role)
                    role_get = discord.utils.get(ctx.guild.roles, name="Unverified")  
                    'await member.remove_roles(role_get)' #caused issue
                await message.channel.send("Congatulations! You have been verified on **" + client.get_guild(i[1]).name + "**." )
                await message.channel.send("Welcome to the Club. Happy CODEing")
        else:
